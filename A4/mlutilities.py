...
def partition(X, T, n, classification=False, shuffle=False):
    ...
    if classification:
        ...
        for c in classes:
            ...
            ntr = l[:ni]
            nte = l[ni:]
            xtrlist.append(X[ntr, :])
            ttrlist.append(T[ntr, :])
            xtelist.append(X[nte, :])
            ttelist.append(T[nte , :])
        xtrlist = np.concatenate(xtrlist)
        ttrlist = np.concatenate(ttrlist)
        xtelist = np.concatenate(xtelist)
        ttelist = np.concatenate(ttelist)
        return xtrlist,ttrlist,xtelist,ttelist
    ...
    nHealthy = round(n*len(healthyI))
    nPark = round(n*len(parkI))
